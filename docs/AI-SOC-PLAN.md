...
| **Version** | 2.8.3 (replan of v1.0; boundary corrected in v2.1; Phases A-D delivered in v2.2-2.5; Phase E delivered in v2.6; Phase F scoped in v2.7 and **delivered in v2.7.1-2.8**) |
| **Supersedes** | Plan v1.0, Summer 2026 |
| **Date** | Summer 2026 |
| **Status** | Re-sequenced against implemented reality, re-scoped against the tool boundary; **Phases A-E complete in `ao-soc` 2.7.0**. **Phase F is complete (F1-F5, `ao-soc` 2.8.4)** — it deepens the pre-action context Phase E's connectors dispatch against (asset criticality, identity role, execution artifacts, reversible actions, a structured decision envelope). M16/M17 (pilot, release) remain engagements rather than build artifacts |
| **Writer** | J.Ekrami |
...
| ~~**R13**~~ | ~~**Target context is coarse until Phase F**~~ *(new, v2.7)* | **Closed v2.7.2** | `action_policy.py`'s `PROTECTED_TARGETS` is an exact-match denylist an operator must type by hand — it has no concept of *what class* a target belongs to. A domain controller, a service account, or a production database not explicitly listed today is indistinguishable, to the risk check, from any other hostname or user. Mitigated today by `ACTION_MAX_AUTOPILOT_RISK` (nothing above it executes without a human) and the precedent gate (nothing autopilots without repeated, reversal-free human confirmation on the same shape) — so the exposure is bounded, not open. **Closed in v2.7.2:** F1 (asset-criticality classifier) stops autopilot on any `CRITICAL` host or address, by name, pattern or subnet, and F2 (identity roles) does the same for a `PRIVILEGED` or `SERVICE` account. *Residual:* both classify by a configured map and naming conventions, not by directory group membership or a CMDB — an asset or account nobody named or patterned is `STANDARD`, which is why `preflight` reports autopilot running with no map file |
| **R14** | **A model id is a tag, not weights** *(new, v2.8)* | Low | Run provenance (F5) records the identifier the provider was *configured* with. `qwen3.5:latest` can be re-pulled to different weights without the name changing, so a run proves which tag produced a decision, not which weights. Mitigated by recording generation parameters and the prompt and reply hashes beside the tag, so a re-run against the stored prompt can be compared. *Residual:* a site that must prove the weights pins its tags to content digests, which the runbook's release checklist now asks about |
...
### Phase F — Pre-action context & reversible response ✅ *(scoped in v2.7; delivered, `ao-soc` 2.8.4)*
...
| Task | Milestone | Addresses | State |
|---|---|---|---|
| F1. Asset-criticality classifier — three-layer lookup (static override map, hostname-pattern rules, CIDR/subnet ranges) producing a `criticality` tag on a target entity; a `CRITICAL` target is refused for autopilot execution by the classifier's own rule, independent of precedent depth | M12/M14 | R13 | ✅ `asset_criticality.py` (stdlib-only, hot-reloaded JSON via `ASSET_CRITICALITY_FILE`; static map is authoritative both ways so it can exempt as well as protect; CIDR matches by *overlap*, so blocking a range that contains a critical subnet is caught). `action_policy.autopilot_allows` refuses a `HIGH_WRITE`-or-above action on a `CRITICAL` target *before* the precedent gate is consulted, and the threshold is a constant, not a setting. A human can still approve it; `READ`/`LOW_WRITE` are unaffected. Stamped per action (`asset_criticality`, `criticality_reason`). Built-in DC/database patterns apply with no file; a broken file never widens anything and `preflight` reports it, and reports autopilot-on with no file at all |
| F2. Identity/privilege enrichment — classify a target account as standard, privileged-admin, or service-account (config map + naming heuristics, e.g. `svc_`/`_svc` prefixes); identity-affecting actions (disable, revoke session, force-reset) are gated the same way F1 gates host actions | M10/M12 | R13 | ✅ `identity_role.py` (stdlib-only, hot-reloaded JSON via `IDENTITY_ROLES_FILE`). Roles `STANDARD` / `PRIVILEGED` / `SERVICE`; static map authoritative both ways, name patterns over the identifier and its short form (`CORP\svc_backup`, `svc_backup@corp.example` → `svc_backup`). A protective entry written with a domain also protects the bare name; an exemption never does. `autopilot_allows` refuses a `HIGH_WRITE`-or-above action on a `PRIVILEGED` or `SERVICE` account, constant not setting, human approval still possible. Stamped per action (`identity_role`, `identity_reason`); `preflight` reports a broken file and autopilot-on with none |
| F3. Execution-artifact fields on the Detection Intake contract — optional `command_line`, `process_guid`, `parent_process` alongside the existing `file_hash`, populated where the source adapter's vendor payload carries them (EDR sources), left empty otherwise. Additive only — the single-detection degenerate case (§4) is unaffected | M03 | — | ✅ `detection.Artifacts`, carried on the detection **beside** the entities and deliberately outside `ENTITY_FIELDS`: a command line is high-cardinality free text and joining on it, or on a GUID whose format differs per vendor, would merge unrelated hosts, so a test asserts identical artifacts on two hosts share no correlation key. Six adapters bumped to 1.1 read them from their vendor's real field names and leave the rest empty (CEF has no standard key; Sentinel supplies a PID, which is not a GUID; Splunk's ambiguous `process` is never promoted). Oversized values are cut and marked, verbatim stays in `raw`. Stored in `detections.artifacts_json`. Reach the model in a fenced `<execution_artifacts>` block declared untrusted — the spec's "CLI arguments" are attacker-controlled text — with silent tools named and their silence not read as benign; a situation with none reads exactly as before |
| F4. Reversible-containment contract — an action with a known inverse (`EDR_ISOLATE`/`EDR_UNISOLATE`, firewall block/unblock) is dispatched with its rollback paired and recorded; an action with no known inverse (session revoke) is flagged non-reversible, and that flag — not a human's guess — is what the case timeline and the autopilot risk class both read | M12 | R12 | ✅ Four states: `REVERSIBLE` (inverse recorded on the action at plan time), `SELF_LIMITING` (revoke session — nothing persistent to undo), `IRREVERSIBLE`, `NOT_APPLICABLE`. **Autopilot refuses an irreversible action** — "a machine only takes actions it can take back" — a constant rather than a setting. A person rolls back with `POST /api/alerts/{id}/actions/{action_id}/rollback`: the inverse rides the *same route* as the original under its own `…:rollback` idempotency key, is claimed atomically so it dispatches once, and a vendor connector with no inverse command refuses rather than re-sending the original (Wazuh ships none). The action's own status is never rewritten; the alert reopens; the case timeline records what ran, whether it can be undone, and who undid it. A rollback deliberately records no outcome — lifting a containment is not a verdict reversal. Also found and fixed on the way: `Unblock IP` classified as a block because it contains "block ip" |
| F5. Structured decision envelope + run provenance — normalize the exported decision shape to `situation` / `decision` / `execution_payload` / `audit_trail`; stamp the `LLMProvider`'s configured model identifier and a hash of the prompt+response on every LLM-sourced decision | M09/M10 | R14 | ✅ `provenance.py` + `model_runs`: every completed model call is recorded **before its output is parsed** (a reply that fails to parse is exactly the one nobody can reconstruct) with provider, model id, parameters, `prompt_sha256`, `response_sha256` and a `reasoning_hash` = `sha256(prompt + 0x00 + response)` that anyone holding the two texts can recompute. Text is kept by default so the proof is checkable (`MODEL_RUN_RETAIN_TEXT`), and integrity is recomputed at export: `verified` / `MISMATCH` / `text_not_retained`. `decision_envelope.py` + `GET /api/alerts/{id}/decision/envelope` exports the four-part shape under a schema version and leaves the dashboard's decision object untouched; confidence is labelled `model_self_reported_uncalibrated` in the payload; autonomy is read from the decision's own record (`PROPOSED` / `SUPERVISED` / `AUTONOMOUS`); the reasoning hash also travels to the executor. A pre-provenance decision exports `model: null` with a note, not a blank |

**DoD — met.** A target's criticality and identity role are
...
hash tying it to what actually ran. Verified, task by task, by `check_asset_criticality`,
`check_identity_roles`, `check_execution_artifacts_contract` / `_flow`, `check_reversibility_policy`,
`check_rollback_contract` / `_flow` and `check_provenance_primitives` / `check_decision_envelope_flow` — each
of whose guards was shown to fail when switched off, not only to pass when on.
...
| 2.7 | Summer 2026 | **Phase F scoped** (planned, not yet delivered). F1 asset-criticality classifier (static map + hostname pattern + CIDR, `CRITICAL` targets refused for autopilot regardless of precedent); F2 identity/privilege enrichment (standard/privileged-admin/service-account) gating identity-affecting actions the same way; F3 execution-artifact fields (`command_line`, `process_guid`, `parent_process`) added to the Detection Intake contract, additive and optional; F4 reversible-containment contract pairing a dispatched action with its inverse where one exists, flagging the rest non-reversible; F5 structured decision envelope (`situation`/`decision`/`execution_payload`/`audit_trail`) with a model identifier and reasoning hash stamped on every LLM-sourced decision. Originated from a review of the project against an external Step-3 pre-action data-input specification. **Explicitly rejected from that same review:** confidence-threshold and time-of-day autopilot gating — reaffirms R6, which closed the confidence-threshold path in v2.5 on measured evidence. §3 (v2.7 note), risk register (**R13 raised**, R6 annotated), §8 Phase F added | J.Ekrami / Claude (Sonnet 5) |
| 2.8.3 | Summer 2026 | **Pilot preparation, part 2: the measurement report** (`ao-soc` 2.8.7; M16 stays 🟡 — the instrument now exists, the pilot has still not run). `orchestrator/pilot_report.py` computes the seven §8 questions of the pilot runbook from the decision store: correlation value, verdict agreement (unchanged / edited / rejected, autopilot separately, reversals overlaid) per detection source, precision per source with sources below a configurable floor flagged for a human, the gate constants replayed over history with a sweep of the precedent count, actions from receipts with rollbacks, autopilot and irreversible dispatches flagged, latency at p50 and p95 (nearest-rank, with an unreliable-p95 flag below the sample minimum), and backup and restore evidence. Properties: opened `mode=ro` and byte-identical afterwards; imports nothing from the broker; every answer carries `answered` / `insufficient_data` / `needs_a_person` and, below `--min-decisions` / `--min-judged`, no number is offered in its place. Stated limits: similarity is not stored, so the gate replay treats (detection source, verdict) as precedent and is an upper bound on openings; a restore leaves only a `.replaced-` file, otherwise a person attests. The report leads with what decides whether it can be believed: model versus rules-fallback share, corrections recorded (none means the pilot has not been run), model-run hashes recomputed, dead letters. Guarded by `check_pilot_report` against a store with known answers, with the read-only property and the percentile rule mutation-checked | J.Ekrami / Claude (Sonnet 5) |
| 2.8.2 | Summer 2026 | **Pilot preparation, part 1: the deployment audited against the code** (`ao-soc` 2.8.6; M16 stays 🟡 — this makes a pilot *possible*, it does not run one). The shipped compose file, `.env.example` and runbook set four names nothing read (`LLM_ENDPOINT`, `LLM_MODEL`, `ALLOWED_ORIGINS`, `DASHBOARD_API_KEYS`) and forwarded only a fixed variable list, so no `CONNECTOR_*`, `MISP_*`, `THEHIVE_*` or `CASE_SYNC_*` setting ever reached the broker and Stages 2-3 of the runbook could not have worked. Fixed (compose forwards `deploy/.env` to the broker only; correct names; maps mounted read-only; `.dockerignore`; pinned dependencies; image tags follow `VERSION`; example file defaults to Stage 0) and guarded three ways: `check_deployment_drift` fails the build when the compose file, example, runbook or README name a setting or metric the code does not read or define; `preflight` reports an unread setting in the environment, with what was probably meant; `deploy/check_env.py` reports a shell variable compose would prefer to `deploy/.env`. The last was found by *rendering* the compose file on a machine running Ollama: its own `OLLAMA_HOST=0.0.0.0:11434` had replaced the GPU host, and the same precedence would have let a stale `TIER2_AUTOPILOT=1` beat a `.env` that read "false". Deploy-level names for the model host are therefore `AOSOC_MODEL_HOST` / `AOSOC_MODEL_PORT`. **Not verified:** an image build, and the suite running inside the Python 3.13 image (the suite passed on 3.14) — Docker was gated in the authoring shell; the runbook now asks for both before Stage 0 | J.Ekrami / Claude (Sonnet 5) |
| 2.8.1 | Summer 2026 | **The dashboard draws Phase F** (`ao-soc` 2.8.5; M13 stays 🟢, the gap it closes is that F1-F5 were API-only). Each action card shows the guard (critical asset, privileged or service account), whether it can be undone and by what, and a **Roll back** control that appears only once the action has run and needs a second, annotated click; a collapsed **Audit trail** shows autonomy level, model, run id, a copyable reasoning hash and the broker's own integrity result, with confidence labelled uncalibrated, and downloads the envelope. Browser-verified end to end (EN and FA/RTL) against a live broker, UI API and Vite. Found and fixed while doing so: a contained incident is where a rollback happens, yet its details page rendered no decision panel, so the panel now lives there too; that page held a once-fetched copy of the incident and kept showing a lifted containment as **CONTAINED**, so it now re-reads after a decision changes; `SIMULATED` and `SUPERSEDED` had no translation and rendered as raw keys. Case events carry `action` / `target` / `connector` / `note` so the timeline is rebuilt in the analyst's language rather than shown as English prose. Node proxy gained the rollback and envelope routes (`testUnify.js` extended). Decided in review: the button appears only when the broker's own state says a rollback can run, yet the broker remains the only enforcement (a connector with no inverse command can still refuse, and the panel then says so) — the button is a convenience, not a control | J.Ekrami / Claude (Sonnet 5) |
| 2.8 | Summer 2026 | **F5 delivered — Phase F complete** (`ao-soc` 2.8.4). `provenance.py` and `model_runs`: every completed model call recorded before its output is parsed, with provider, model id, parameters, prompt and reply SHA-256 and a `reasoning_hash` over both that is recomputable with no code of ours; text retained by default so the proof can be re-checked, and integrity recomputed at export (`verified` / `MISMATCH` / `text_not_retained`). `decision_envelope.py`: `GET /api/alerts/{id}/decision/envelope` exports `situation` / `decision` / `execution_payload` / `audit_trail` under `ao-soc.decision-envelope/1`, with the paired rollback per action, evidence pointers, `autonomy_level` from the decision's own record, and confidence labelled uncalibrated in the payload; the dashboard's own decision object is untouched. `reasoning_hash` also reaches the executor. `model_runs` added to the backup manifest. Decided in review: `model_id` is a configured tag, not weights (**R14 raised**, Low); recording a run is best-effort so an audit-table fault cannot cost a decision. Pilot runbook updated: the asset and identity files are now prerequisites for Stage 4, and what autopilot cannot do by construction is stated. §3 unchanged (no milestone status moves), risk register (R14), Phase F marked delivered | J.Ekrami / Claude (Sonnet 5) |
| 2.7.4 | Summer 2026 | **F4 delivered** (`ao-soc` 2.8.3). Every disruptive action is classified `REVERSIBLE` (inverse recorded at plan time), `SELF_LIMITING`, `IRREVERSIBLE` or `NOT_APPLICABLE`; **autopilot refuses an irreversible action**, no existing autopilot scenario depended on one. A human rolls back via `POST /api/alerts/{id}/actions/{action_id}/rollback`: same route as the original, own stable `…:rollback` idempotency key, atomic claim, `SIMULATED` under a dry run, and a connector with no inverse (Wazuh, by default) refuses rather than re-send the original. The alert reopens, the action's status is not rewritten, and the case timeline records execution, undo-ability and who undid it. Decided in review: a rollback records no outcome (routine release after resolution is not a reversal); `Reset password` is `IRREVERSIBLE` while `Revoke session` is `SELF_LIMITING`, so the requirement's example autonomous actions split — session revocation still runs, password reset now waits. Fixed a pre-existing hazard: an inverse verb such as `Unblock IP` classified as the block it undoes. New columns on `alert_soar_actions` (auto-migrated, legacy rows `UNASSESSED`); connectors log/webhook/wazuh bumped. Both diagram pairs updated: gate 3 now includes reversibility and a rollback panel added | J.Ekrami / Claude (Sonnet 5) |
| 2.7.3 | Summer 2026 | **F3 delivered** (`ao-soc` 2.8.2). `detection.Artifacts` (`command_line`, `process_guid`, `parent_process`) on the Detection Intake contract, carried beside the entities and **outside the correlation vocabulary** — a command line is free text and must not join unrelated hosts (asserted by a test). Wazuh/Sysmon, CrowdStrike, Elastic ECS, Sentinel, Splunk and native read them from their vendor's real field names (adapters 1.1); CEF carries none, Sentinel's PID is not presented as a GUID, Splunk's ambiguous `process` is not promoted. Oversized values are cut and marked with the verbatim payload kept. Stored in `detections.artifacts_json` (auto-migrated). Delivered to the model in a fenced, explicitly-untrusted `<execution_artifacts>` block, because command lines are attacker-controlled input to a model that helps decide containment; tools that reported nothing are named and their silence is not read as benign. Decided in review, not implemented: the requirement's "cannot proceed to action generation without these" is **not** enforced as a blocker — many detections (network, identity) have no process, and refusing them would drop exactly the alerts that need a human. Header, Phase F table updated; diagram footers moved | J.Ekrami / Claude (Sonnet 5) |
| 2.7.2 | Summer 2026 | **F2 delivered** (`ao-soc` 2.8.1). `identity_role.py` — `STANDARD` / `PRIVILEGED` / `SERVICE` from a static map (authoritative both ways) and name patterns over the identifier and its short form, hot-reloaded from `IDENTITY_ROLES_FILE`. `autopilot_allows` refuses a `HIGH_WRITE`-or-above action on a privileged or service account, a constant rather than a setting, with human approval intact. A protective static entry written with a domain also protects the bare name detections report; an exemption never does, so it cannot leak across domains. Found by the test, not by review: the first version did not do this. Verdict stamped per action; preflight reports a broken file and autopilot-on with none. **R13 closed** (residual: name conventions and a configured map, not directory groups). Header, Phase F table, R13 updated; both diagram pairs updated for gate 4 | J.Ekrami / Claude (Sonnet 5) |
| 2.7.1 | Summer 2026 | **F1 delivered** (`ao-soc` 2.8.0). `asset_criticality.py` — static map / hostname patterns / CIDR ranges classifying a target `STANDARD` or `CRITICAL` with no external CMDB; static entries are authoritative both ways (they can exempt), CIDR matches by overlap, the file hot-reloads, a broken file never widens anything and is reported by `preflight`. `action_policy.autopilot_allows` refuses any `HIGH_WRITE`-or-above action on a `CRITICAL` target ahead of the precedent gate; the threshold is a constant, not a setting, and a human can still approve. Verdict stamped on each action (`asset_criticality`, `criticality_reason`). Tested end to end: a 99 % `CONTAIN` on `DC-01` stays `PENDING` even with the precedent gate off. Header, Phase F table, R13 (half closed) updated; both diagram pairs updated for the new autopilot refusal | J.Ekrami / Claude (Sonnet 5) |
