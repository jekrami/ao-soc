...
**Version:** 2.8.5 — see `VERSION` at repo root (bump on every release).
...
| GET    | `/api/incidents/:id/decision/envelope`              | The decision as `situation` / `decision` / `execution_payload` / `audit_trail` (F5) |
| POST   | `/api/incidents/:id/actions/:actionId/rollback`     | Take back one executed action (F4). 404 · 409 not run / already undone · 422 no inverse |
...
- **v2.8.5 — The dashboard draws Phase F.** Until now F1-F5 existed only as API fields, so an analyst could not see *why* a plan was waiting for a person or take an action back without a terminal. Each action card now shows **Critical asset** / **Privileged account** / **Service account** (what autopilot will never touch, with the rule that decided it on hover), **Can be undone** with the inverse verb, **Cannot be undone**, or **Nothing to undo**, and a **Roll back** button that appears only once the action has actually run and needs a second, annotated click. An **Audit trail** section (collapsed, fetched on open) shows autonomy level, model, run id, a copyable reasoning hash and the integrity result the broker recomputed, with the model's confidence labelled uncalibrated, plus a JSON download of the decision envelope. The decision panel now also lives on the incident details page, because a contained incident is exactly where a containment gets lifted; and that page re-reads the incident afterwards, so a lifted containment stops being displayed as a standing one. The case file rebuilds the new `action` / `rollback` entries in the analyst's language from structure, not from English prose. EN and FA (RTL). Also fixed: `SIMULATED` and `SUPERSEDED` had no translation and rendered as raw keys.
- **v2.8.4 — Phase F5, and Phase F is complete: what produced a decision, provably.** Every model call is now recorded the instant it returns — *before* its output is parsed, because a reply that fails to parse is exactly the one nobody can reconstruct — as a `model_runs` row: provider, model id, generation parameters, `prompt_sha256`, `response_sha256`, and a `reasoning_hash` over both that anyone holding the two texts can recompute in one line. The text is kept by default so the proof can be re-checked (`MODEL_RUN_RETAIN_TEXT=0` keeps only the hashes and the export then says so); tampering with a stored run is reported as `MISMATCH`, not returned as plausible. `GET /api/alerts/{id}/decision/envelope` exports one decision as `situation` / `decision` / `execution_payload` / `audit_trail` — MITRE, correlated source alerts, autonomy level, each action with its target type, destination tool and paired rollback, the evidence and the model — with the model's confidence labelled uncalibrated in the payload. The reasoning hash also travels to the executor. The dashboard's own decision object is untouched. What it cannot prove: the model id is a configured *tag*, not weights. See `orchestrator/README.md`.
- **v2.8.3 — Phase F4: a machine only takes actions it can take back.** Every disruptive action is now classified `REVERSIBLE` (with its inverse recorded beside it at plan time — `Block IP` → `Unblock IP`), `SELF_LIMITING` (a revoked session needs no undo) or `IRREVERSIBLE` (kill process, password reset, a vague "contain"). **Autopilot refuses an irreversible action** at any confidence; the reversible ones still run. A person can take a reversed action back with `POST /api/alerts/{id}/actions/{action_id}/rollback`: the inverse verb travels the *same route* as the original with its own idempotency key, a vendor connector with no inverse command refuses rather than re-sending the original, the alert reopens instead of staying "contained", and the case timeline records what ran, whether it can be undone, and who undid it. Also fixed: `Unblock IP` used to classify as a block, because it contains "block ip". See `orchestrator/README.md`.
- **v2.8.2 — Phase F3: what actually ran.** The Detection Intake contract gains an optional `artifacts` object — `command_line`, `process_guid`, `parent_process` — filled by six adapters (Wazuh/Sysmon, CrowdStrike, Elastic ECS, Sentinel, Splunk, native) where the vendor payload really carries them and left empty where it does not (CEF has no standard key; Sentinel gives a PID, which is not a GUID and is not presented as one). They sit *beside* the entities and are **never a correlation key**: two hosts running the same installer must not become one situation. They reach the model in a fenced, explicitly-untrusted block — a command line is attacker-controlled text about to be read by a model that decides whether to contain something — and a tool that reported no execution record is named, with its silence not read as benign. A situation with no artifacts reads exactly as before. See `orchestrator/README.md`.
- **v2.8.1 — Phase F2: what an account *is*.** `identity_role.py` classifies an account as `STANDARD`, `PRIVILEGED` or `SERVICE` from a static map (authoritative both ways) and name patterns, reducing `CORP\svc_backup` and `svc_backup@corp.example` to one name. A disable, revoke or reset on a privileged or service account is **never executed by autopilot** — the first removes the responder, the second silently stops a business process — while a human can still approve it. Same guard as F1, same independence from confidence and precedent, same hot-reloaded file, same preflight reporting. Built-in conventions apply with no setup. Closes R13. See `orchestrator/README.md`.
- **v2.8.0 — Phase F begins: what a target *is*, not just what it is called.** `asset_criticality.py` classifies a host or address through three layers that need no CMDB — a static map (authoritative both ways, so it can exempt as well as protect), hostname patterns, and CIDR ranges — into `STANDARD` or `CRITICAL`. An action of class `HIGH_WRITE` or above on a `CRITICAL` asset is **never executed by autopilot**: not at 99% confidence, not with a full precedent record, not in the confidence-only lab mode, and not by configuration — a human can still approve it from the card. It sits *before* the precedent gate so it does not depend on it. The verdict is stamped on each action beside its risk class, the file hot-reloads, a broken file never widens anything and is reported by preflight. Built-in domain-controller and database patterns apply with no setup. Closes half of R13 (F2 covers identities). See `orchestrator/README.md` and `docs/AI-SOC-PLAN.md` §8 Phase F.
...
**Version:** 2.8.0 (see `VERSION` — increment on each release commit)
