...
export const SystemHealthPage: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-base font-semibold text-fg">System Health</h1>
        <p className="text-[11px] text-muted">Live telemetry from the AO-SOC pipeline</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Pipeline Status</CardTitle>
          <CardSubtitle>End-to-end data flow</CardSubtitle>
        </CardHeader>
        <CardBody>
      ...
      {systemHealth && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <Stat title="Total EPS"         value={fmtNumber(systemHealth.splunk.events_per_sec)}     icon={Database} />
          <Stat title="Avg Inference"     value={`${systemHealth.llm.inference_latency_ms}ms`}     icon={Brain} />
          <Stat title="SOAR Success Rate" value={`${systemHealth.soar.success_rate}%`}             icon={Workflow} />
          <Stat title="Broker Queue"      value={systemHealth.broker.queue_depth}                  icon={Server} />
          <Stat title="GPU Temperature"   value={`${systemHealth.gpu.temperature_c}°C`}            icon={Cpu} />
          <Stat title="Playbooks Running" value={systemHealth.soar.playbooks_running}              icon={Activity} />
        </div>
      )}
    </div>
  );
...
const PipelineDiagram: React.FC<{ status: Record<string, 'ONLINE' | 'DEGRADED' | 'OFFLINE' | 'UNKNOWN'> }> = ({ status }) => {
  const nodes = [
    { key: 'splunk', label: 'Splunk',          icon: Database },
    { key: 'broker', label: 'AI Broker',       icon: Workflow },
    { key: 'llm',    label: 'Local LLM',       icon: Brain },
    { key: 'soar',   label: 'SOAR / Response', icon: Network }
  ];
  return (
