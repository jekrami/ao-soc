...
import {
  Server, Cpu, HardDrive, Network, Gauge, Workflow, Activity, Database, Brain
} from 'lucide-react';
import { ResponsiveContainer, AreaChart, Area } from 'recharts';

// Rolling buffer of health samples for sparklines
...
export const SystemHealthPanel: React.FC = () => {
  ...
  // Push a sample each render that has new data
  useMemo(() => {
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardSubtitle>Live telemetry · refreshes every 5s</CardSubtitle>
          </div>
        </div>
        <button
        ...
        {!systemHealth ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            <HealthCell
              ...
              stats={[
                { label: 'EPS',          value: fmtNumber(systemHealth.splunk.events_per_sec) },
                { label: 'Correlations', value: `${systemHealth.splunk.correlations_per_min}/m` }
              ]}
            ...
            <HealthCell
              ...
              stats={[
                { label: 'Queue',   value: systemHealth.broker.queue_depth },
                { label: 'Uptime',  value: `${systemHealth.broker.uptime_hours}h` }
              ]}
            />
            <HealthCell
              title="Ollama (LLM)"
              ...
              stats={[
                { label: 'Latency',  value: `${systemHealth.llm.inference_latency_ms}ms` },
                { label: 'Tokens/s', value: systemHealth.llm.tokens_per_sec }
              ]}
            ...
            <HealthCell
              ...
              stats={[
                { label: 'Util',  value: `${systemHealth.gpu.utilization_pct}%` },
                { label: 'VRAM',  value: `${systemHealth.gpu.vram_used_gb.toFixed(1)}/${systemHealth.gpu.vram_total_gb}GB` },
                { label: 'Temp',  value: `${systemHealth.gpu.temperature_c}°C` }
              ]}
            ...
            <HealthCell
              ...
              stats={[
                { label: 'Running', value: systemHealth.soar.playbooks_running },
                { label: 'Queued',  value: systemHealth.soar.playbooks_queued }
              ]}
            />
            <HealthCell
              title="Pipeline"
              ...
              status="ONLINE"
              stats={[
                { label: 'Correlations', value: `${samples.length ? samples[samples.length - 1].corr : 0}/m` }
              ]}
